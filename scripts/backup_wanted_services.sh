...
echo nodepool_status=$nodepool_status, expect 0
echo zuul_status=$zuul_status, expect 4
echo mysql_status=$mysql_status, expect 1
echo zk_status=$mysql_status, expect 1
echo apache_status=$apache_status, expect 1

if [ $nodepool_status -eq 0 ] && [ $zuul_status -eq 4 ] && [ $mysql_status -eq 1 ] && [ $zk_status -eq 1 ] && [ $apache_status -eq 1 ];then
    echo "OK"
